...
def get_image_from_ampl(amplitudes, gradient_max, gradient_min):
    ...
    for amplitude_index in range(len(amplitudes)):
        ...
        for time_index in range(len(amplitudes[0])):
            ...
            x = (amplitude - gradient_min) / (gradient_max - gradient_min)
            color_triplet = interpolate_or_clip(turbo_colormap_data, x)
            pixel = list(map(lambda cc: round(255 * cc), color_triplet))
